...
const Hero = () => {
  const { sortedData, setSortedData, empData } = ContextData();
  ...
  useEffect(() => {
    let updatedItem = empData.filter((curEle) => {
      return curEle.firstName == `${name}`;
    });
    setSortedData(updatedItem[0]);
  ...
  return (
    <motion.div
      ...
      className=" pb-20 bg-slate-950 text-white min-h-screen mybg"
      id="home"
    >
